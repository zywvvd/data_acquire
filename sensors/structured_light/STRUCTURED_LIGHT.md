...
> **符号约定**(贯穿全文)。`P=(X,Y,Z)` 物点三维坐标(mm);`Z` 深度,物点沿光轴距离(mm);`X,Y` 横纵坐标(mm);`f` 焦距(像素,= 物理焦距 ÷ 像素尺寸);`B` 基线,两光心横向间距(双目=左右相机,单目=相机↔投射器,mm);`d` 视差,同一物点在两图中的水平位置差(px);`x_L,y_L / x_R,y_R` P 在左/右图像坐标(px);`u_c / u_p` 单目的相机像列/投射器已知投射列(px);`σ_d` 子像素视差噪声(px);`ΔZ` 每 1 视差像素对应的深度增量(mm)。坐标系:原点在左相机光心(双目)或相机光心(单目),`+Z` 沿光轴(深度方向),`+X` 沿基线方向,`+Y` 纵向;像坐标以主点为原点。量纲自洽:`Z=f·B/d → px·mm/px=mm`。
...
相机成像的基础是针孔模型:将相机简化为一个光心 O 与一个像面,物点 P 发出的光经 O 在像面上落点 p 成像。建立相机坐标系,O 为原点,Z 轴沿光轴,像面位于 `z=f` 处,f 为焦距。在 X-Z 平面内,物点 P=(X,Z) 的光线 OP 与像面 `z=f` 的交点 p 的横坐标为 x,由相似三角形(小三角形 O-f-x 与大三角形 O-Z-X):
...
投影把三维坐标 (X,Y,Z) 压成二维 (x,y),Z 在此过程中消失——同一条光线 OP 上远近不同的物点都投到同一个像点 p。因此单张图像无法区分「近处的小物」与「远处的大物」,这是单目不能测距的根本原因,也是需要双目(两个观察点)的动机。
...
- P 在左相机的像横坐标 `x_L = f · X / Z`
- P 在右相机坐标系横坐标为 (X−B),像横坐标 `x_R = f · (X − B) / Z`
...
视差每差 1 像素(`|dd|=1`),深度差 `ΔZ = Z² / (f·B)`。深度被视差量化,因 `ΔZ ∝ Z²`,深度精度随距离平方退化——此为双目(及一切三角测距)的几何属性。

实际匹配为子像素(sub-pixel):SGBM 通过块相关度的亚像素插值,视差精度可达 0.1 像素量级。设子像素视差噪声为 σ_d,则深度噪声 `RMS ≈ σ_d · Z² / (f·B)`。σ_d 由匹配算法与纹理质量决定,与距离近似无关,故深度噪声继承 `ΔZ ∝ Z²`。

### 1.6 对应问题、整流与视差搜索窗(量程)

整流(rectification):实际两相机并不严格平行同行,出厂标定对左右图各做一次单应变换,映射到「对应点同行」的标准姿态,使立体匹配从二维搜索降为沿同一行的一维搜索(epipolar 约束)。

![整流:把两幅图变换到对应点同行,匹配由二维搜索降为一维](assets/rectification.png)

视差搜索窗:SGBM 只在有限视差窗 `[d_min, d_max]` 内搜索(视差越大搜索越慢越易越界,视差过小则三角失效),超出此窗则匹配失败、深度无效。代入 `Z=fB/d`,该窗对应一段有限的工作量程:

```
Z_min = f·B / d_max     (近端:物体太近,视差超出窗上界)
Z_max = f·B / d_min     (远端:物体太远,视差低于窗下界)
```
...
两者之差 `d = u_c − u_p = f·B / Z`,即 `Z = f·B / d`,与双目同形。差异仅在于视差 d 的获取:
...
u_p 已知化的三种主流方案:空间编码图案(每投射像素图案唯一,如 De Bruijn 序列、Gray 码条纹,单次拍摄解码);相移条纹(多幅相移正弦条纹求相位,近场精度可达微米级);固定散斑 + 参考图(Kinect v1 的 light-coding,以参考散斑图充当第二幅图做块匹配)。单目与双目的主要差异:
...
精度仍为 `ΔZ=Z²/(f·B)`——三角几何不区分「相机↔相机」与「相机↔投射器」。现代消费级深度相机多采用主动双目,因一片廉价的随机散斑投射器(无需编码、无需标定)配两台相机即可,制造简单、对投射器漂移较鲁棒;本设备选用双目属工程取舍,并非单目不可行。
...
- 焦距 f = 1102.09 px,基线 B = 99.89 mm,`f·B ≈ 110 080 mm²`。
- 视场 `HFOV=2·atan(640/1102.09)=60.2°`、`VFOV=2·atan(480/1102.23)=47.1°`;点云呈以相机为顶点的扇形径向展开即此视场锥的几何表现。
- 视差示例(§1.3 的 d=fB/Z):1 m 处 d≈110 px,0.6 m 处 d≈187,4 m 处 d≈27。
- 量程预测(§1.6):实测视差窗 `DISPARITY_OFFSET=25`、`DISPARITY_NUM=162`,即 d∈[25,187];`Z_min=110080/187≈589 mm≈0.59 m`,`Z_max=110080/25≈4403 mm≈4.41 m`。
...
正对平墙,在不同距离各采 3 帧(1280×960),以点云到拟合平面的残差衡量平整度。方法关键在只统计墙体上的点:远距时视场溢出墙体,地板/天花板等非墙点会污染结果(曾因此将 2.5 m 的 RMS 误算为 94 mm)。先由深度直方图定墙的尖峰(轴向深度众数),取 ±120 mm 窗剔远处背景;再对窗内点以 MAD(中位绝对偏差)鲁棒迭代拟合主平面并校验正对(`|法向·z|>0.9`);取 3×MAD 内点为真墙点,重拟合后算 RMS/P95。各距离结果存于 `data/fm815_precision/d{0.5..4.5}m/flatness.txt`。
...
**理论印证**:由 §1.5,RMS=σ_d·Z²/(fB)。以实测 RMS 反推 σ_d(上表末列):1–3.3 m 段 σ_d=0.114–0.138 px,中位 0.126 px,近似恒定。单次立体匹配的视差抖动稳定在约 0.13 像素、与距离无关,深度噪声完全继承 `ΔZ∝Z²`。图中虚线为理论曲线 `0.126·Z²/(fB)`,与实测点吻合——精度随距离平方退化可由 f、B、σ_d 推导并被数据印证。

近端 0.63 m 处 σ_d=0.181 偏高(传感器读噪声等贡献约 0.47 mm 近端地板);远端 3.3–3.7 m 处 σ_d 降至 0.099、RMS 趋于平台(12.3→12.5 mm):近远端极限时较差匹配率先变无效被剔,存活匹配质量较好,RMS 不再增长但有效率下滑(90%→78%→75%)。综合得完整模型 `RMS(Z)≈√((σ_d·Z²/(fB))²+z_floor²)`,σ_d≈0.126 px,z_floor≈0.47 mm。多帧时间稳定性:逐像素深度时间抖动约 0.5 mm,相邻帧残差相关 ≈1.0,表明单帧 RMS 的主体为固定系统性图案(视差量化网格 + 整流残余),时间随机成分仅约 0.5 mm,多帧平均对精度改善有限。
...
§1.3 指出散斑是对应问题所需的人工纹理来源。对照实验(同摆位,640×480,各 3 帧平均):LASER power=100/auto=1(出厂频闪)深度有效率 33.4%;power=100/auto=0(手动常亮)33.2%;power=0(关闭)2.4%。关闭投射器后深度由 33% 降至 2%,印证散斑是匹配纹理的主要来源、对应问题在此无纹理则无解。

`LASER_POWER` 持久化存储,实验后须以 `-laser 100 -lauto 1` 恢复(否则下次裸采集沿用残留 power=0 致深度归零)。`main.cpp` 启动打印 `LASER now: power=X auto=Y` 供核查。

### 3.4 点云密度(稀疏)的来源

深度有效率不高有五个来源,按可调性排列:(a) 场景与量程(影响最大)——前景 <0.59 m 整块无效(§1.6),平放桌面有效率 6.8%,摆位使目标落入 0.6–4 m 后升至 33%(640)或更高(1280);(b) 分辨率模式——默认 640×480 为 1280×960 的降采样(佐证:640 模式 fx=522.43 恰为 1280 的 fx 之半),改 1280×960 点数 ×4、精度不降;(c) SGBM 严苛度——默认 `UNIQUE_FACTOR=80`+`LRC=on` 偏精度,放宽至 uniq=20、关 LRC 则有效率 33%→47% 但精度下降;(d) IR 增益——出厂默认 gain=32 最优,扫描 32/64/128/200→33.5%/25.8%/1.3%/0%,调高反而变差;(e) 表面材质——玻璃/屏幕/黑色/强反光面打不回 IR 散斑,恒无效。
...
`SimpleView_CaptureDump -ip 192.168.1.114 -n 6 -dmode 1280 -outdir <dir>`。要点:depth 用 1280×960;SGBM 保留默认,仅在需密度时用 `-uniq 20 -nolrc`;gain 保持 32(改过须重置);摆位使目标落入 0.6–4 m,避开屏幕/玻璃/强反光面;按精度选距(§3.2),毫米级装 1–2 m。`main.cpp` 开关:`-dmode`、`-ire/-irg`、`-uniq/-nolrc`、`-ir`(同开 IR,见 §4.3)、`-laser/-lauto`、`-nodepth`(纯 IR)、`-irflash`(本设备无效);每帧统计深度有效率。
