...
**关于"焦距相同"的假设**:左右两个镜头的物理焦距不可能完全相等($f_L\ne f_R$)。上面的推导实际是在**整流后**的图像上做的——整流(§1.6)把左右两幅图重采样到同一个虚拟相机(公共焦距 $f$、共面像面、平行光轴),所以这里的 $f$ 是整流后的公共焦距,并非要求两只镜头一致。即便暂且假设光轴平行、只有焦距不同,原始视差也会退化为

$$
d=x_L-x_R=\frac{f_L X}{Z}-\frac{f_R(X-B)}{Z}=\frac{(f_L-f_R)X+f_R B}{Z}
$$

它同时含 $X$ 和 $Z$,无法由视差单值解出深度 $Z$。这正是必须先整流(统一焦距 + 共面平行)、再做一维匹配的几何原因,也把本节与 §1.6 连了起来。
...
**R/T 与单应变换的关系**(回答「是单应还是 R/T 位姿调整」):**两者都对,层面不同**。$R/t$(相对旋转 + 平移)是整流的**几何起因**——标定测出两相机间的 3D 相对姿态;但整流在**图像层面**的实现是单应变换:将相机绕光心做一次虚拟 3D 旋转,该旋转作用到图像上的效果恰好是一个 2D 单应矩阵 $H = K\,R_{\text{rect}}\,K^{-1}$($K$ 为内参矩阵)。链条:$R/t$(3D 位姿)→ 算出所需的虚拟旋转 $R_{\text{rect}}$ → 该旋转在图像上诱导出单应 $H$(2D)→ 用 $H$ 对图像做 warp 重采样。整流算法(Bouguet 法)将旋转在两相机间均匀分配以减少重采样失真,为左右图各算出一个 $H$。

**焦距如何被统一——公共虚拟相机**：上面 $H=K\,R_{\text{rect}}\,K^{-1}$ 两端是**同一个 $K$**,只把朝向摆正,**并不改变焦距**(整流后左图仍是 $f_L$、右图仍是 $f_R$,二者依旧不等)。要真正统一焦距,需把输出端换成**公共内参 $K_{\text{new}}$**:

$$
H_L=K_{\text{new}}\,R_{\text{rect}}\,K_L^{-1},\qquad H_R=K_{\text{new}}\,R_{\text{rect}}\,R\,K_R^{-1}
$$

读法(从右往左):$K_i^{-1}$ 把原图像素还原成归一化光线方向(这一步剥掉相机 $i$ 自己的焦距 $f_i$),$R_i$ 把光线转到公共朝向,$K_{\text{new}}$ 用**公共焦距 $f_{\text{new}}$** 重新投回像素。由于 $H_L$、$H_R$ 输出端共用同一个 $K_{\text{new}}$,两张整流图都被重采样进同一个虚拟相机,焦距统一为 $f_{\text{new}}$——镜头本身不动,「改焦距」只是每个像素按 $H$ 重新落位、新像素网格改用 $f_{\text{new}}$ 计量。

$f_{\text{new}}$ 的取值是算法自选的自由参数:取 min 可保证视场不越界但损失分辨率,取 max 则反之。Bouguet 法 / OpenCV `stereoRectify` 默认取**两相机焦距的平均**(左右 fx、fy 四值平均,并令新图 fx=fy 以保持正方形像素),使整流图像素尺度尽量贴近原图、重采样失真最小。基线呢?整流后两相机除投影中心在水平方向错开 $f_{\text{new}}B$ 外,所有内参全同(同一 $f_{\text{new}}$、同一主点、共面平行)——这个水平错开正是对应点的视差 $d$,故 $Z=fB/d$ 中只剩一个 $f$(即这里的 $f_{\text{new}}$)。

**实现**:对每个像素,整流 = 镜头去畸变(非线性,校正径向/切向畸变)+ 单应 warp(线性,虚拟旋转)的**复合变换**。通常预计算为一张像素重映射表(lookup map),运行时一次性重采样,无需逐帧重算。OpenCV 的 `stereoRectify` + `initUndistortRectifyMap`、本设备出厂固件均走此流程。
